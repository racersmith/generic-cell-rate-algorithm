...
class TestThrottleMethods:
    ...
    def test_normalize_rate_limit(self):
        ...
        assert (
            throttle.normalize_rate_limit(rate_limit, state, total_allocation=100).count
            == 333
        )
        assert (
            throttle.normalize_rate_limit(rate_limit, state, total_allocation=150).count
            == 222
        )
        assert (
            throttle.normalize_rate_limit(rate_limit, state, total_allocation=200).count
            == 166
        )
        assert (
            throttle.normalize_rate_limit(rate_limit, state, total_allocation=300).count
            == 111
        )
        assert (
            throttle.normalize_rate_limit(rate_limit, state, total_allocation=1e9).count
            == 1
        )
    ...
    def test_calculate_next_tat(self):
        ...
        assert (
            throttle.calculate_next_tat(now, state, rate_limit)
            == now + rate_limit.inverse
        )

        state = throttle.ThrottleState(tat=now + 3)
        assert (
            throttle.calculate_next_tat(now, state, rate_limit)
            == now + 3 + rate_limit.inverse
        )
    ...
    def test_get_usage_rate_limit(self):
        ...
        assert rate == burst, "Expected burst rate"

        sustained.usage = 25
        rate = throttle.get_usage_rate_limit([burst, sustained])
        assert rate == sustained, "Expected sustained rate"
    ...
    def test_enforce_single_throttle_state(self):
        ...
        assert (
            throttle.enforce_single_throttle_state([throttle_state]) == throttle_state
        )
        assert (
            throttle.enforce_single_throttle_state((throttle_state,)) == throttle_state
        )
...
class TestGCRA:
    def test_rolling_period(self, time):
        ...
        gcra = throttle.GCRA(rate_io=rate_io, throttle_io=throttle_io, time=time)
        ...
        assert average_rate <= rate_limit.rate, "Throttled rate too high."
        assert average_rate > 0.95 * rate_limit.rate, "Throttled rate too low."
...
class TestGcraMultiRate:
    def test_normal(self, time):
        ...
        gcra = throttle.GcraMultiRate(
            rate_io=rate_io, throttle_io=throttle_io, time=time
        )
        ...
        try:
            ...
            pytest.fail("Did not correctly limit function")

        for limit, period_log in endpoint.period_log.items():
            for i, count in period_log.items():
                assert count < limit.count, (
                    f"Exceeded limit of {limit.count}/{limit.period} in period: {i} with {count} calls"
                )
        ...
        assert count / dt > sustained_rate.rate, "Throttle not allowing burst rates."
...
class TestGcraPriority:
    def test_normal(self, time):
        ...
        assert high_priority_rate > low_priority_rate, (
            "Higher priority should have faster requests."
        ...
        assert current_wait_time > 0, (
            f"Something is amiss, wait times should be positive. {current_wait_time}"
        )
